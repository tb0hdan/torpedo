...
func GiphyProcessMessage(api *slack.Client, event *slack.MessageEvent) {
	var message string
	var params slack.PostMessageParameters
	var giphyResponse GiphyResponse

	_, command, message := GetRequestedFeature(event.Text)
	if command != "" {
		query := url.QueryEscape(command)
		result, err := GetURLBytes(fmt.Sprintf("http://api.giphy.com/v1/gifs/search?q=%s&api_key=dc6zaTOxFJmzC", query))
		if err != nil {
